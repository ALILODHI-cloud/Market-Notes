...
Banks face two parallel capital regimes: a **risk-based** one (capital ÷ risk-weighted assets) and a **leverage** one (capital ÷ total leverage exposure). For the eight US GSIBs the leverage stack carried a flat **2%** add-on on top of the 3% minimum (= 5%). The reform replaced that flat 2% with **50% of each bank's GSIB surcharge** — the surcharge being the extra, risk-based capital charge scaled to a bank's systemic importance — so the requirement now scales by bank rather than a one-size-fits-all 2%. The effective leverage minimum falls from **5% to ~3.5–4.25%**, with the explicit aim of returning the eSLR to a **backstop** rather than a binding constraint, so it stops penalizing low-risk, balance-sheet-intensive activity such as **Treasury intermediation and repo financing**. The effect: expanded dealer balance-sheet capacity.
